...
from scarabs.mora.models.recall_with_dcl import RecallWithDCL, RecallWithDCLConfig
from scarabs.task_factory import TaskFactoryWithTabularRecall

task = TaskFactoryWithTabularRecall()
...
# config = RecallWithDCLConfig.from_pretrained("config.json")
# task.create_feature2transformer_and_config(data_args, training_args, config)

# Train
parser = HfArgumentParser((ModelArguments, DataArguments, TrainArguments))  # type: ignore
model_args, data_args, training_args = parser.parse_json_file("arguments.json")
config = RecallWithDCLConfig.from_pretrained(
    os.path.join(data_args.dataset_cache, "meta")
)
model = RecallWithDCL(config)
inputs = {}
