...
from scarabs.mora.models.ctr_with_widedeep import CtrWithWideDeep, CtrWithWideDeepConfig
...
config = CtrWithWideDeepConfig.from_pretrained("config.json")
...
parser = HfArgumentParser((ModelArguments, DataArguments, TrainArguments))  # type: ignore
model_args, data_args, training_args = parser.parse_json_file("arguments.json")
config = CtrWithWideDeepConfig.from_pretrained(
    os.path.join(data_args.dataset_cache, "meta")
)
model = CtrWithWideDeep(config)  # type: ignore
