...
from scarabs.mora.models.ctr_with_edcn import CtrWithEDCN, CtrWithEDCNConfig
...
# config = CtrWithEDCNConfig.from_pretrained("config.json")
...
parser = HfArgumentParser((ModelArguments, DataArguments, TrainArguments))  # type: ignore
model_args, data_args, training_args = parser.parse_json_file("arguments.json")
config = CtrWithEDCNConfig.from_pretrained(
    os.path.join(data_args.dataset_cache, "meta")
)
model = CtrWithEDCN(config)  # type: ignore
# Print the parameter count for each layer
