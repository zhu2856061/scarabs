...
from scarabs.mora.models.ctr_with_sam import CtrWithSAM, CtrWithSAMConfig
...
# config = CtrWithSAMConfig.from_pretrained("config.json")
...
parser = HfArgumentParser((ModelArguments, DataArguments, TrainArguments))  # type: ignore
model_args, data_args, training_args = parser.parse_json_file("arguments.json")
config = CtrWithSAMConfig.from_pretrained(os.path.join(data_args.dataset_cache, "meta"))
model = CtrWithSAM(config)  # type: ignore
inputs = {}
