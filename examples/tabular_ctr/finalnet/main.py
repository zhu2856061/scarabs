...
from scarabs.mora.models.ctr_with_finalnet import CtrWithFinalNet, CtrWithFinalNetConfig
...
config = CtrWithFinalNetConfig.from_pretrained("config.json")
...
parser = HfArgumentParser((ModelArguments, DataArguments, TrainArguments))  # type: ignore
model_args, data_args, training_args = parser.parse_json_file("arguments.json")
config = CtrWithFinalNetConfig.from_pretrained(
    os.path.join(data_args.dataset_cache, "meta")
)

model = CtrWithFinalNet(config)  # type: ignore
inputs = {}
