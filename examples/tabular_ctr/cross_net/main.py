...
from scarabs.mora.models.ctr_with_dcn import CtrWithDCN, CtrWithDCNConfig
...
config = CtrWithDCNConfig.from_pretrained("config.json")
...
parser = HfArgumentParser((ModelArguments, DataArguments, TrainArguments))  # type: ignore
model_args, data_args, training_args = parser.parse_json_file("arguments.json")
config = CtrWithDCNConfig.from_pretrained(os.path.join(data_args.dataset_cache, "meta"))
model = CtrWithDCN(config)  # type: ignore
inputs = {}
