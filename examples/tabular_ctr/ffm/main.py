...
from scarabs.mora.models.ctr_with_ffm import CtrWithFFM, CtrWithFFMConfig
...
# config = CtrWithFFMConfig.from_pretrained("config.json")
...
parser = HfArgumentParser((ModelArguments, DataArguments, TrainArguments))  # type: ignore
model_args, data_args, training_args = parser.parse_json_file("arguments.json")
config = CtrWithFFMConfig.from_pretrained(os.path.join(data_args.dataset_cache, "meta"))
model = CtrWithFFM(config)
inputs = {}
