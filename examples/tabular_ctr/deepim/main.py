...
from scarabs.mora.models.ctr_with_deepim import CtrWithDeepIM, CtrWithDeepIMConfig
...
config = CtrWithDeepIMConfig.from_pretrained("config.json")
...
parser = HfArgumentParser((ModelArguments, DataArguments, TrainArguments))  # type: ignore
model_args, data_args, training_args = parser.parse_json_file("arguments.json")
config = CtrWithDeepIMConfig.from_pretrained(
    os.path.join(data_args.dataset_cache, "meta")
)
model = CtrWithDeepIM(config)  # type: ignore
